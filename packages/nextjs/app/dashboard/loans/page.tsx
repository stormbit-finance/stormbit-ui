  ...
  return (
    <div className="flex flex-col items-center mx-6">
      <div className="w-full flex justify-end my-4">
        <select className="bg-transparent text-white px-4 py-2 border rounded-[8px]" onChange={handleFilterChange} value={filter}>
      ...
      {filteredLoans.map((loan) => (
        <div key={loan.id} className="grid grid-cols-6 gap-4 mt-4 w-full bg-[#2D2D2D] rounded-[5px]">
          ...
          <div className={`col-span-1 text-center text-[#AE9FFD] flex items-center justify-center ${loan.status === 'Pending' ? 'text-yellow-400' : loan.status === 'Executed' ? 'text-green-400' : ''}`}>
